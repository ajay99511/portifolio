...
import { ChevronLeft, ChevronRight, Layout, Cpu, ArrowLeft, ChevronDown, ChevronUp } from "lucide-react";
...
const WalkthroughEngine = ({ project }: WalkthroughEngineProps) => {
  ...
  if (steps.length === 0) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center text-zinc-400 font-mono text-xs uppercase tracking-widest px-4 text-center">
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-black text-white pt-16 sm:pt-20">
      {/* Left Pane: Technical Briefing — collapsible on mobile */}
      <aside className="w-full lg:w-[380px] xl:w-[400px] border-b lg:border-b-0 lg:border-r border-zinc-800 flex flex-col overflow-y-auto shrink-0">
        <div className="p-4 sm:p-6 lg:p-8">
          <Link href="/#archive" className="flex items-center gap-2 text-zinc-500 hover:text-white transition-colors mb-6 sm:mb-8 lg:mb-12 font-mono text-xs uppercase tracking-widest group py-1">
          ...
          <div className="mb-4 lg:mb-8">
            ...
            <h1
              className="font-bold uppercase tracking-tighter mb-3 sm:mb-4"
              style={{ fontSize: "var(--text-3xl)" }}
            >
              {project.title}
            </h1>
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed hidden lg:block">{project.longDescription}</p>
          </div>
      ...
      <main className="flex-1 p-3 sm:p-6 lg:p-12 flex flex-col relative bg-[#050505] min-h-[50vh] lg:min-h-0">
        <div className="flex-1 relative flex flex-col">
          {/* Simulated Viewport Chrome — simplified on mobile */}
          <div className="bg-zinc-900 border border-zinc-800 rounded-t-lg sm:rounded-t-xl p-2 sm:p-3 flex items-center gap-3 sm:gap-4">
          ...
          <div className="flex-1 border-x border-b border-zinc-800 bg-[#0A0A0A] overflow-hidden relative min-h-[300px] sm:min-h-[400px]">
            ...
            {/* Floating Tactical Console — responsive width */}
